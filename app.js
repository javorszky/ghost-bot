...
(function() {
  ...
  Creator = require('./hubot/src/creator');
  ...
  if (Options.create) {
    ...
    adapterPath = Path.join(__dirname, "hubot", "src", "adapters");
    ...
    loadScripts = function() {
      ...
      scriptsPath = Path.resolve(__dirname, "hubot", "src", "scripts");
          ...
          return Fs.readFile(hubotScripts, function(err, data) {
            ...
            if (data.length > 0) {
              try {
                ...
                scriptsPath = Path.resolve(__dirname, "node_modules", "hubot-scripts", "src", "scripts");
                return robot.loadHubotScripts(scriptsPath, scripts);
              } catch (err) {
                return console.error("Error parsing JSON data from hubot-scripts.json: " + err);
              }
            }
          });
        }
          ...
          return Fs.readFile(externalScripts, function(err, data) {
            ...
            if (data.length > 0) {
              try {
                ...
                return console.error("Error parsing JSON data from external-scripts.json: " + err);
              }
            }
          });
        }
