...
(function() {
  var Adapter, Campfire, CampfireStreaming, EnterMessage, EventEmitter, HTTPS, LeaveMessage, Robot, TextMessage, _ref,
  ...
  _ref = require('../message'), TextMessage = _ref.TextMessage, EnterMessage = _ref.EnterMessage, LeaveMessage = _ref.LeaveMessage;
  ...
  Campfire = (function(_super) {
    ...
    Campfire.prototype.send = function() {
      ...
      if (strings.length > 0) {
        return this.bot.Room(envelope.room).speak(strings.shift(), function(err, data) {
          if (err != null) {
            _this.robot.logger.error("Campfire error: " + err);
          }
          return _this.send.apply(_this, [envelope.user].concat(__slice.call(strings)));
        });
      }
    };
    ...
    Campfire.prototype.topic = function() {
      ...
      return this.bot.Room(envelope.room).topic(strings.join(" / "), function(err, data) {
        if (err != null) {
          return _this.robot.logger.error("Campfire error: " + err);
        }
      });
    };
    ...
    Campfire.prototype.run = function() {
      ...
      bot.on("TextMessage", withAuthor(function(id, created, room, user, body, author) {
        if (bot.info.id !== author.id) {
          return self.receive(new TextMessage(author, body, id));
        }
      }));
      ...
      bot.Me(function(err, data) {
        var roomId, _i, _len, _ref1, _results;
        bot.info = data.user;
  ...
  CampfireStreaming = (function(_super) {
        ...
        listen: function() {
          ...
          request = HTTPS.request(options, function(response) {
                ...
                while ((offset = buf.indexOf("\r")) > -1) {
                  ...
                  if (part) {
                    try {
                      ...
                      _results.push(logger.error("Campfire error: " + error + "\n" + error.stack));
                    }
                  } else {
            ...
            return response.on("error", function(err) {
              return logger.error("Campfire response error: " + err);
            });
          });
    ...
    CampfireStreaming.prototype.request = function(method, path, body, callback) {
      var headers, logger, options, request;
      logger = this.robot.logger;
      headers = {
